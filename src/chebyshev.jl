...
function chebyshev(N,mom,abm)
  ...
  ab(1,1)=abm(1,1)+mom(2)/mom(1);
  ab(1,2)=mom(1);
  if N==1
    return [ab, normsq(1)=mom(1)];
  end
  sig(1,1:2*N)=0; sig(2,:)=mom(1:2*N);
  for n=3:N+1
    for m=n-1:2*N-n+2
      sig(n,m)=sig(n-1,m+1)-(ab(n-2,1)-abm(m,1))*sig(n-1,m) -ab(n-2,2)*sig(n-2,m)+abm(m,2)*sig(n-1,m-1);
    end
    ab(n-1,1)=abm(n-1,1)+sig(n,n)/sig(n,n-1)-sig(n-1,n-1)/sig(n-1,n-2);
    ab(n-1,2)=sig(n,n-1)/sig(n-1,n-2);
  end
  for n=1:N
    normsq(n)=sig(n+1,n);
  end;
  normsq=transpose(normsq);
  return [ab,normsq]
  end
